...
import { toast, ToastContainer } from "react-toastify"; // Aggiungi ToastContainer qui
...
// Plugin per immagine background
const plugin = {
...
const Dashboard = () => {
  ...
  // Funzione per fetch dei dati per le chart
  ...
  useEffect(() => {
    fetchData(); // Fetch data quando il componente è montato
  }, []);
  ...
  // Funzione per gestire la navigazione verso l'admin
  const navigateToAdmin = () => {
    // Aggiungi un controllo per verificare che `userData` e `roleId` esistano
    if (userData && userData.roleId === 2) {
      // Supponendo che il ruolo admin sia identificato da roleId 2
      navigate("/admin");
    } else {
      // Mostra il toast di errore se l'utente non ha i permessi
      toast.error("❌ Non hai i permessi per accedere alla pagina!");
    }
  };

  // Funzione per gestire la navigazione verso l'area utente
  ...
  return (
    ...
    >
      ...
      {/* Alert di errore */}
      {showAlert && (
        <div className="bg-red-500 text-white text-center p-4 mb-4 rounded-lg">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        {userData && (
          <div className="bg-gray-400 shadow p-4 rounded-full mb-8 flex flex-col items-center justify-center text-center flex-1 h-full">
            ...
            <p className="text-lg">
              <strong>Username: nunzia</strong> {userData.username}
            </p>
            <p className="text-lg">
              <strong>Email: nunzia.biele@virgilio.it</strong> {userData.email}
            </p>
            <p className="text-lg">
              <strong>Numero:3486966996</strong> {userData.phone}
            </p>
          </div>
        )}
      ...
      {/* Aggiungi ToastContainer qui per i messaggi di errore */}
      <ToastContainer />
    </div>
  );
};
